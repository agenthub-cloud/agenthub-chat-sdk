...
/** Java 协议 SDK RunEventTypes 的完整 v1 → legacy 映射。 */
export const RUN_EVENT_TYPE_MAP: Readonly<Record<string, string>> = Object.freeze({
  ...
  'ai.run.context.compacted': 'context_cleaned',
...
export function normalizeRunEvent(
  ...
  const type = eventV1.type === 'ai.run.context.compacted' && eventV1.data?.kind === 'overflow_trimmed'
    ? 'context_overflow_trimmed'
    : RUN_EVENT_TYPE_MAP[eventV1.type] || eventV1.type
  return { ...(eventV1.data || {}), type }
}
